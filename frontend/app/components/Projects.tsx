...
function Projects() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Batch entrance animation for all cards - much more performant
      gsap.fromTo(
        Array.from(cards),
        {
          opacity: 0,
...
const ProjectCard = memo(({ project }: { project: typeof projects[0] }) => {
  ...
  return (
    ...
    >
      ...
      <div className={`absolute inset-0 bg-gradient-to-br ${project.gradient} opacity-0 group-hover:opacity-20 transition-opacity duration-200`}></div>

      {/* Header Section */}
      <div className={`relative bg-gradient-to-br ${project.gradient} p-12 text-center overflow-hidden`}>
        <div className="relative z-10">
          <div className="mb-4 flex justify-center">
            <div className="transform group-hover:scale-110 transition-transform duration-200 inline-block">
